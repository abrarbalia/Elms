...
// ───────────────── FILE STORAGE ─────────────────
// UPLOADS_DIR is injected by electron/main.js so it is always an absolute,
// writable path in both dev and packaged production mode.
const uploadDir = process.env.UPLOADS_DIR || path.join(__dirname, '..', 'uploads');
if (!fs.existsSync(uploadDir)) fs.mkdirSync(uploadDir, { recursive: true });
console.log('[Server] uploads dir:', uploadDir);
...
// ───────────────── DATABASE ─────────────────
const mongoUri = process.env.MONGO_URI || "mongodb+srv://Elms:FGaBrIn63uPDG57p@cluster0.umivhxn.mongodb.net/employeeDB";
...
// ACTIVE SESSION (IMPORTANT)
app.get('/api/active-session', async (req, res) => {
  const session = await Session.findOne().sort({ updatedAt: -1 });
  res.json(session || { sessionName: "2025-26", startDate: "", endDate: "" });
});
...
app.post('/api/staff', async (req, res) => {
  ...
  const user = new User({ ...req.body, "Employee Code": nextCode });
  await user.save();
...
app.get('/api/leaves/admin', async (req, res) => {
  res.json(await Leave.find().sort({ From: -1 }));
});

// STAFF LEAVES
app.get('/api/leaves/staff/:empCode', async (req, res) => {
  const empCode = Number(req.params.empCode);
  res.json(await Leave.find({ Emp_CODE: empCode }));
});
...
app.get('/api/partialadmin/all', async (req, res) => {
  res.json(await Leave.find()); // adjust if needed
});
